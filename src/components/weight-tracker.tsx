...
export function WeightTracker({ logs }: { logs: WeightLog[] }) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        {latest && (
          <span className="text-sm">
            ...
            {diff !== null && diff !== 0 && (
              <span className="ml-1.5" style={{ color: diff > 0 ? "var(--danger)" : "#16a34a" }}>
                {diff > 0 ? "+" : ""}
                {diff} kg
              </span>
            )}
      ...
      {chartData.length >= 2 ? (
        <WeeklyLineChart data={chartData} color="var(--accent)" unit="kg" />
      ) : (
      ...
      >
        <input
          ...
          className="flex-1 rounded-xl border border-border bg-surface-muted px-3.5 py-2.5 text-sm outline-none focus:border-accent"
        />
        <button
          type="submit"
          disabled={pending}
          className="rounded-xl border-[1.5px] border-accent px-4 py-2.5 text-sm font-semibold text-accent transition-colors hover:bg-accent hover:text-white disabled:opacity-40"
        >
          Noter
        </button>
      </form>
