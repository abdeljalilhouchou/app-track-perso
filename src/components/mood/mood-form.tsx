...
export function MoodForm({ today, existing }: { today: string; existing: Existing }) {
  ...
  return (
      ...
      action={(formData) =>
        startTransition(async () => {
          ...
          const r = await run(() => logMood(formData), {
            success: existing ? "Humeur du jour mise à jour" : "Humeur du jour enregistrée",
            failure: "Humeur non enregistrée",
          });
          if (!r || r.error) return;
          ...
          if (score <= 2) {
            toast.info("Une journée difficile ? Ce que tu ressens compte. Pense à une pause, de l'eau ou une courte marche.", {
              title: "Prends soin de toi",
              duration: 7000,
            });
          } else if (score === 5) {
            toast.success("Belle journée ! Note ce qui l'a rendue si bonne pour la reproduire.", { title: "Super humeur 😄" });
          }
        })
      }
    ...
    >
      ...
      >
        {pending ? "Enregistrement..." : saved ? "Enregistré ✓" : existing ? "Mettre à jour" : "Enregistrer"}
      </motion.button>
    </form>
  );
}
